...
@section('content')
    <section class="h-screen w-screen grid grid-cols-2">
        ...
        <div class="max-h-screen max-w-full bg-[#F4F4F4] pl-24 pt-12 pr-16 pb-16 overflow-auto" x-data="{ isTaCChecked: false }">
            <form action="{{ route('pemesananExplore4') }}" method="POST">
                @csrf
                <div class="mt-6 mb-4">
                    <h1 class="text-3xl font-bold">Data Diri Pemesan</h1>
                </div>
                <div class="bg-white rounded-lg shadow mb-5 px-6 py-4">
                    <h2 class="text-gray text-lg font-semibold mb-3">DETAIL PESERTA</h2>
                ...
                <div class="bg-white rounded-lg shadow mb-5 px-6 py-4">
                    <h2 class="text-gray text-lg font-semibold mb-3">Lokasi Pertemuan</h2>
                    <div class="flex flex-col gap-5">
                        <div>
                            <h4 class="text-lg font-medium">{{ $meeting_point->title }}</h4>
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-gray">{{ $meeting_point->description }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow mb-5 px-6 py-4" x-data="{ count: 0, useCoin: false, coin: {{ \Illuminate\Support\Facades\Auth::user()->coin }}, useVoucher: false, voucher: '', voucherList: {{ str_replace('"', "'", json_encode($voucher)) }}, totalPrice: @json($total_price), ticketPrice: {{ $total_price }}, get totalPay() { return this.totalPrice - ((this.useCoin) ? this.coin : 0) - ((this.useVoucher) ? this.voucherList.find((vc) => vc.id == this.voucher).actual_disc : 0) }  }">
                    ...
                    <button x-bind:class="'rounded px-4 py-2 text-center text-white block w-full mt-4 mb-2 ' + ((isTaCChecked) ? 'bg-primary transition border border-primary hover:bg-transparent hover:text-primary' : 'bg-gray')" x-bind:disabled="!isTaCChecked" @click="count = 1" x-show="count == 0">Bayar</button>
                    <div class="flex justify-center items-center px-4 py-2 mt-4 mb-2 rounded bg-gray text-white" x-show="count == 1">
