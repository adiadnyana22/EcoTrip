    ...
    <section class="h-screen w-screen grid grid-cols-2">
        <div class="shadow-xl max-h-screen max-w-full pl-24 pt-12 pr-16 sticky top-0 left-0" x-data="{ modelOpen: false }">
            <div class="flex items-center gap-8">
                <a href="{{ route('viewWaste2') }}"><i class='bx bx-chevron-left text-4xl'></i></a>
        ...
        <div class="max-h-screen max-w-full bg-[#F4F4F4] pl-24 pt-12 pr-16 pb-16 overflow-auto">
            <form action="{{ route('mtdWaste3') }}" method="POST" x-data="{ star : 0 }">
                    ...
                    <div class="px-6 py-4">
                        <div class="flex gap-3 text-4xl mt-2 mb-6 ml-1" x-data="{ starHover: 0 }">
                            <label>
                                ...
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-gray" x-bind:class="(star >= 1 || starHover >= 1) ? 'text-star' : ''" x-on:mouseenter="starHover = 1" x-on:mouseleave="starHover = 0">
                                    <i class='bx bxs-star' ></i>
                                </div>
                            </label>
                            <label>
                                <input class="sr-only peer" name="star" type="radio" value="2" x-model="star" />
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-gray" x-bind:class="(star >= 2 || starHover >= 2) ? 'text-star' : ''" x-on:mouseenter="starHover = 2" x-on:mouseleave="starHover = 0">
                                    <i class='bx bxs-star' ></i>
                                </div>
                            </label>
                            <label>
                                <input class="sr-only peer" name="star" type="radio" value="3" x-model="star" />
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-gray" x-bind:class="(star >= 3 || starHover >= 3) ? 'text-star' : ''" x-on:mouseenter="starHover = 3" x-on:mouseleave="starHover = 0">
                                    <i class='bx bxs-star' ></i>
                                </div>
                            </label>
                            <label>
                                <input class="sr-only peer" name="star" type="radio" value="4" x-model="star" />
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-gray" x-bind:class="(star >= 4 || starHover >= 4) ? 'text-star' : ''" x-on:mouseenter="starHover = 4" x-on:mouseleave="starHover = 0">
                                    <i class='bx bxs-star' ></i>
                                </div>
                            </label>
                            <label>
                                <input class="sr-only peer" name="star" type="radio" value="5" x-model="star" />
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-gray" x-bind:class="(star >= 5 || starHover >= 5) ? 'text-star' : ''" x-on:mouseenter="starHover = 5" x-on:mouseleave="starHover = 0">
                                    <i class='bx bxs-star' ></i>
                                </div>
                            </label>
                        </div>
                ...
                <div class="flex justify-end">
                    ...
                    <button class="rounded-lg px-4 py-2 text-white" x-bind:class="(star == 0) ? 'bg-gray' : 'border border-primary bg-primary transition hover:bg-transparent hover:text-primary'" x-bind:disabled="star == 0">Selanjutnya</button>
                </div>
            </form>
        </div>
    </section>
