...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Ubah Meeting Point</h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('adminExploreMeetingEditMethod', $point->id) }}">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label class="form-label">Title</label>
                    <input type="text" class="form-control" name="title" value="{{ $point->title }}"  placeholder="Title ..." required>
                    @error('title')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Description</label>
                    <input type="text" class="form-control" name="description" value="{{ $point->description }}"  placeholder="Description ..." required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="map" value="{{ $point->mapIframe }}"  placeholder="Map Iframe ..." required>
