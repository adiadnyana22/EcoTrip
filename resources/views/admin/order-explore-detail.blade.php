...
@section('title', 'EcoTrip Order Explore')

@section('content')
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Order Explore</h1>
    </div>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Detail Order Explore</h6>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="row py-3">
                        <div class="col-6">
                            <b>Explore Name</b> : {{ $order->explore->name }}
                        </div>
                        <div class="col-6">
                            <b>Account Email</b> : {{ $order->user->email }}
                        </div>
                        <div class="col-6">
                            <b>Type</b> : {{ $order->explore->type == 0 ? 'Private Trip' : 'Open Trip' }}
                        </div>
                        <div class="col-6">
                    ...
                    <div class="row py-2">
                        <div class="col-12">
                            ...
                            <div class="mb-2">
                                <b>Qty Local Ticket</b> : {{ $order->qty_indonesia }} x {{ Carbon\Carbon::parse($order->date)->isWeekend() ? number_format($order->explore->local_weekend_price) : number_format($order->explore->local_price) }}
                            </div>
                            <div class="mb-2">
                                <b>Qty Foreign Ticket</b> : {{ $order->qty_foreign }} x {{ Carbon\Carbon::parse($order->date)->isWeekend() ? number_format($order->explore->foreign_weekend_price) : number_format($order->explore->foreign_price) }}
                            </div>
                            <div class="mb-2">
                    ...
                    <div class="mt-2">
                        ...
                        <form action="{{ route('adminOrderExploreConfirmMethod', $order->id) }}" method="POST">
                            @method('PUT')
                            @csrf
            ...
            <a href="{{ route('adminOrderExplore') }}" class="btn btn-info float-right mr-3 mt-2">Kembali</a>
        </div>
    </div>
@endsection
