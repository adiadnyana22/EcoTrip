...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Ubah EcoExplore</h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('adminExploreEditMethod', $explore->id) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label class="form-label">Name</label>
                    <input type="text" class="form-control" name="name" value="{{ $explore->name }}" placeholder="Name ..." required>
                    @error('name')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Type</label>
                    <select name="type" id="type" class="form-control" required>
                        <option value="0" {{ $explore->type == 0 ? 'selected' : '' }}>Private Trip</option>
                        <option value="1" {{ $explore->type == 1 ? 'selected' : '' }}>Open Trip</option>
                    </select>
                    @error('type')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Local Weekday Price</label>
                    <input type="number" class="form-control" name="local_weekday" value="{{ $explore->local_price }}" placeholder="Local Weekday Price ..." required>
                    @error('local_weekday')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Local Weekend Price</label>
                    <input type="number" class="form-control" name="local_weekend" value="{{ $explore->local_weekend_price }}" placeholder="Local Weekend Price ..." required>
                    @error('local_weekend')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Foreign Weekday Price</label>
                    <input type="number" class="form-control" name="foreign_weekday" value="{{ $explore->foreign_price }}" placeholder="Foreign Weekday Price ..." required>
                    @error('foreign_weekday')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Foreign Weekend Price</label>
                    <input type="number" class="form-control" name="foreign_weekend" value="{{ $explore->foreign_weekend_price }}" placeholder="Foreign Weekend Price ..." required>
                    @error('foreign_weekend')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Location</label>
                    <input type="text" class="form-control" name="location" value="{{ $explore->location }}" placeholder="Location ..." required>
                    @error('location')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Description</label>
                    <textarea name="description" id="description" class="form-control" rows="10" placeholder="Description ...">{{ $explore->description }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="activity" id="activity" class="form-control" rows="10" placeholder="Activity ...">{{ $explore->activity }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="includes" id="includes" class="form-control" rows="10" placeholder="Includes ...">{{ $explore->includes }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="itinerary" id="itinerary" class="form-control" rows="10" placeholder="Itinerary ...">{{ $explore->itinerary }}</textarea>
                    @error('itinerary')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-3">
                    <img src="{{ asset('assets/user/images/explore/'.$explore->picture) }}" alt="EcoExplore" class="w-25 d-block mb-2">
                    ...
                    <div class="row justify-content-center">
                        @foreach ($exploreImages as $image)
                            <div class="col-3 position-relative">
                                <img src="{{ asset('assets/user/images/explore/'.$image->picture) }}" alt="EcoExplore" class="w-100" style="height: 150px; object-fit: cover; border: 1px solid #1f1f1f; border-radius: 1rem">
                                <div class="position-absolute" style="top:-.5rem; right:-.25rem">
                                    <a href="{{ route('adminExploreImageDeleteMethod', $image->id) }}" class="px-2 py-2 bg-danger text-white border-0"><i class="fas fa-fw fa-trash"></i></a>
                                </div>
                            </div>
                        @endforeach
                    </div>
